...
abstract public class Creature implements Runnable,FundamentalBattle{
    ...
    Creature(BattleField field, Controller controller)
    {
        ...
        shield.setFitWidth(80);
        shield.setOpacity(0);//设置透明度
        status = Status.ALIVE;
        ...
        try {
            shield.setImage(new Image("/image/shield.png"));
        }catch (Exception e){
        }
    }
    ...
    public ImageView getShield() { return shield; }
    ...
    public synchronized void beAttacked(int dv)
    {
        ...
        {
            history.add("D");
            controller.DefendDisplay(this);//播完无敌动画
            return;
        }
        //如果没死，则判定为闪避攻击
        int hurt_point = 0;
        ...
        {
            controller.failBeAttackedDisplay(this);//播放闪避动画
        }
        else
        {
            hurt_point = dv;
